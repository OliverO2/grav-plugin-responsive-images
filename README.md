...
To use alternative _background_ images, a JavaScript auto-discovery script must set a class on the `<html>` DOM element with the alternative format's name (e.g. `<html class="webp">`). For WebP, such a script can be generated by [Modernizr](https://modernizr.com/download?webp-setclasses&q=webp) and must be inserted as inline JavaScript into each page. 

Additional information can be found here: [Using WebP Images | CSS-Tricks](https://css-tricks.com/using-webp-images/).
